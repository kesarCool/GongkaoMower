...
弹窗脚本目录建议：`Assets/Script/Game/App/UI/` 或 `Assets/Script/Game/Shell/UI/`（与 `Common/README` 中「业务 UI 不放 Common」一致）。
...
1. 保持现有战斗逻辑；`EnemyWordLabel` 等 TMP 仅在 **BattleLoading 完成后** 再依赖中文字体（或 prefab 默认字体占位，Loading 里统一替换）。
...
2. **首包资源**：用微信开发者工具「代码依赖分析」看首包体积；确认无 `msyh`、无大 `.bytes`（若刻意移出）。  
